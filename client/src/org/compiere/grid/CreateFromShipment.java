...
public class CreateFromShipment extends CreateFrom 
{
	...
	protected Vector<Vector<Object>> getOrderData (int C_Order_ID, boolean forInvoice)
	{
		...
		Boolean response;
		/*  Beta Code for VAT Checking  Done on 17/12/2017 by Ancy Jose     */
		BigDecimal areaid=DB.getSQLValueBD(null, "SELECT nvl(c_salesarea_id,0) FROM c_order WHERE c_order_id=?", C_Order_ID);
		String TaxOld=DB.getSQLValueString(null,"SELECT nvl(taxoldyn,'N') FROM c_order WHERE c_order_id=?", C_Order_ID);
		Vector<Vector<Object>> data = new Vector<Vector<Object>>();
		
		if (TaxOld.equalsIgnoreCase("N"))
		{
				String TaxYn;
				if (areaid.doubleValue()>0)
					TaxYn=DB.getSQLValueString(null,"SELECT taxyn from c_salesarea WHERE c_salesarea_id=?",areaid);
				else
					TaxYn="Y";
				if (TaxYn.equalsIgnoreCase("Y"))
				{
					int count = DB.getSQLValue(null,"SELECT count(*) FROM c_orderline col inner join m_product mp"
		              + " ON mp.m_product_id=col.m_product_id INNER JOIN c_tax ct ON ct.c_taxcategory_id=mp.c_taxcategory_id "
		             +" WHERE col.c_tax_id != ct.c_tax_id AND c_order_id=?",C_Order_ID);
		                    
					if(count>0)
					{
						int Wndow=Env.WINDOW_INFO;
						
						System.out.println("********Desktop Version********** ");
						//boolean respon = FDialog.ask(Wndow,  null,"Tax is not updated on the order ","Tax is not updated on the order, Please revise the order to update the tax. (OR) Select a proper purchase category on the order.");
						
						//ADialog.warn(0,null,"Tax is not updated on the order, Please revise the order to update the tax.");
						//log.saveError("Error", null, "Tax is not updated on the order, Please revise the order to update the tax.");
						return data;
						
					}   
				}
		}
        /* End of Beta Code for VAT Checking  Done on 17/12/2017 by Ancy Jose     */
