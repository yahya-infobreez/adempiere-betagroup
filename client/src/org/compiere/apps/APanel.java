...
import org.compiere.grid.VSelectActuatorFactory;
...
//		aNew.setEnabled(!changed && insertRecord);
	...
	private void cmd_zoomAcross()
	{
		...
		new AZoomAcross (aZoomAcross.getButton(), 
			m_curTab.getTableName(),m_curTab.getAD_Window_ID(), query);
	}	//	cmd_zoom
...
//				batch = vda.isBatch();
		...
		else if (col.equals("SelectActuator"))
		{
			// Run form only if the button has no process defined - teo_sarca [ 1974354 ] 
			if (vButton.getProcess_ID() <= 0)
			{
				ISelectActuator cf = VSelectActuatorFactory.create(m_curTab);
				if(cf != null)
				{
					if(cf.isInitOK())
					{
...
//			if(table_ID == 1000018){
			if(m_curTab.getTableName().equalsIgnoreCase("C_PROJECTRECEIPT")){
				...
				else if(ps != null && ps.equals("N")){
					if(ADialog.ask(m_curWindowNo, this, "PostImmediate?")){
					String sql = "{call proc_post_projectreceipt(?) }";
					try {
					    CallableStatement cstmt = DB.prepareCall(sql);
			            cstmt.setInt(1, m_curTab.getRecord_ID());
			            cstmt.executeUpdate();
			            cmd_refresh();
			            cstmt.close();
			        	} 
			...
			//Beta Journal Voucher Added by Subair Dtd:20.06.2012
			if(m_curTab.getTableName().equalsIgnoreCase("BETA_JOURNAL")){
				Object ps = m_curTab.getValue("Posted");
				if(ps != null && ps.equals("Y")){
					new org.compiere.acct.AcctViewer (Env.getContextAsInt (m_ctx, m_curWindowNo, "AD_Client_ID"),
							m_curTab.getAD_Table_ID(), m_curTab.getRecord_ID());					
			...
			//Beta BPartner Journal Voucher Added by Subair Dtd:15.10.2012
			if(m_curTab.getTableName().equalsIgnoreCase("BETA_BP_JOURNAL")){
				Object ps = m_curTab.getValue("Posted");
				if(ps != null && ps.equals("Y")){
					new org.compiere.acct.AcctViewer (Env.getContextAsInt (m_ctx, m_curWindowNo, "AD_Client_ID"),
							m_curTab.getAD_Table_ID(), m_curTab.getRecord_ID());					
...
//  C_QUOTATION POST BY BETA
			
			if(m_curTab.getTableName().equalsIgnoreCase("C_QUOTATION")){
				Object ps = m_curTab.getValue("Posted");
				if(ps != null && ps.equals("Y")){
					new org.compiere.acct.AcctViewer (Env.getContextAsInt (m_ctx, m_curWindowNo, "AD_Client_ID"),
							m_curTab.getAD_Table_ID(), m_curTab.getRecord_ID());					
					...
					if(ADialog.ask(m_curWindowNo, this, "PostImmediate?")){
					String sql = "{call Beta_proc_post_quotation(?) }";
					try {
					    CallableStatement cstmt = DB.prepareCall(sql);
			            cstmt.setInt(1, m_curTab.getRecord_ID());
			            cstmt.executeUpdate();
...
//C_QUOTATION POST CODE END	

//WARRANTY LETTER POST BY BETA
			
			if(m_curTab.getTableName().equalsIgnoreCase("BETA_WARRANTYLTR")){
				Object ps = m_curTab.getValue("Posted");
				if(ps != null && ps.equals("Y")){
					new org.compiere.acct.AcctViewer (Env.getContextAsInt (m_ctx, m_curWindowNo, "AD_Client_ID"),
							m_curTab.getAD_Table_ID(), m_curTab.getRecord_ID());					
					...
					if(ADialog.ask(m_curWindowNo, this, "PostImmediate?")){
					String sql = "{call Beta_proc_post_warrantyltr(?) }";
					try {
					    CallableStatement cstmt = DB.prepareCall(sql);
			            cstmt.setInt(1, m_curTab.getRecord_ID());
			            cstmt.executeUpdate();
...
//WARRANTY LETTER POST CODE END	
			
//SUBMITTAL POST BY BETA
			
			if(m_curTab.getTableName().equalsIgnoreCase("BETA_SUBMITTAL")){
				Object ps = m_curTab.getValue("Posted");
				if(ps != null && ps.equals("Y")){
					new org.compiere.acct.AcctViewer (Env.getContextAsInt (m_ctx, m_curWindowNo, "AD_Client_ID"),
							m_curTab.getAD_Table_ID(), m_curTab.getRecord_ID());					
					...
					if(ADialog.ask(m_curWindowNo, this, "PostImmediate?")){
					String sql = "{call Beta_proc_post_submittal(?) }";
					try {
					    CallableStatement cstmt = DB.prepareCall(sql);
			            cstmt.setInt(1, m_curTab.getRecord_ID());
			            cstmt.executeUpdate();
...
//SUBMITTAL POST CODE END	
			
//COMPLIANCE STMT POST BY BETA
			
			if(m_curTab.getTableName().equalsIgnoreCase("BETA_COMPLSTMT")){
				Object ps = m_curTab.getValue("Posted");
				if(ps != null && ps.equals("Y")){
					new org.compiere.acct.AcctViewer (Env.getContextAsInt (m_ctx, m_curWindowNo, "AD_Client_ID"),
							m_curTab.getAD_Table_ID(), m_curTab.getRecord_ID());					
					...
					if(ADialog.ask(m_curWindowNo, this, "PostImmediate?")){
					String sql = "{call Beta_proc_post_ComplStmt(?) }";
					try {
					    CallableStatement cstmt = DB.prepareCall(sql);
			            cstmt.setInt(1, m_curTab.getRecord_ID());
			            cstmt.executeUpdate();
...
//COMPLIANCE STMT POST CODE END				
		
			
//XX_PDC_PAYMENT
			if(m_curTab.getTableName().equalsIgnoreCase("XX_PDC_PAYMENT")){
				Object ps = m_curTab.getValue("Posted");
				Object pdc_status = m_curTab.getValue("PDC_STATUS");
				if(ps != null && ps.equals("Y")){
					new org.compiere.acct.AcctViewer (Env.getContextAsInt (m_ctx, m_curWindowNo, "AD_Client_ID"),
							m_curTab.getAD_Table_ID(), m_curTab.getRecord_ID());					
				}
				// For Under Collection
				else if(ps != null && ps.equals("N") && pdc_status != null && pdc_status.equals("U")){
					if(ADialog.ask(m_curWindowNo, this, "PostImmediate?")){
					String sql = "{call proc_post_PDC(?) }";
					try {
					    CallableStatement cstmt = DB.prepareCall(sql);
			            cstmt.setInt(1, m_curTab.getRecord_ID());
			            cstmt.executeUpdate();
				...
				//FOR Cancellation
				else if(ps != null && ps.equals("N") && pdc_status != null && pdc_status.equals("C")){
					if(ADialog.ask(m_curWindowNo, this, "PostImmediate?")){
					String sql = "{call proc_pdc_cancel_post(?) }";
					try {
					    CallableStatement cstmt = DB.prepareCall(sql);
			            cstmt.setInt(1, m_curTab.getRecord_ID());
			            cstmt.executeUpdate();
				...
				//FOR Bounced
				else if(ps != null && ps.equals("N") && pdc_status != null && pdc_status.equals("B")){
					if(ADialog.ask(m_curWindowNo, this, "PostImmediate?")){
					String sql = "{call proc_pdc_bounced_post(?) }";
					try {
					    CallableStatement cstmt = DB.prepareCall(sql);
			            cstmt.setInt(1, m_curTab.getRecord_ID());
			            cstmt.executeUpdate();
			            ...
			            String sql_pay = "select c_payment_id from xx_pdc_payment " +
			            		" where xx_pdc_payment_id = ? ";
			            Integer c_payment_id = DB.getSQLValue(null, sql_pay, m_curTab.getRecord_ID());
			             
			            if (c_payment_id == null || c_payment_id == 0)
			            	return;
			            
			            MPayment payment = new MPayment(m_ctx,c_payment_id,null);
