...
public class ProjectClose extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		//**********************************Validate Finish Date ** Added by Subair on 25.10.2012*********************** 
		String sqlChk = "Select datefinish from c_project where c_project_id=? "; 
		Timestamp DateFin = DB.getSQLValueTS(get_TrxName(), sqlChk, m_C_Project_ID);
   	 	
		if (DateFin == null)
		{
			addLog(" Finish Date of the Project should not be blank");
	 		return " ";
		}
   	 	//**************************************************************************************************************
		...
		String sql = "{call proc_close_project(?) }";
		try {
         ...
         //   int updprj = DB.executeUpdate(sql,null);
            cstmt.close();
        	} 
        catch (Exception e) 
        	{
