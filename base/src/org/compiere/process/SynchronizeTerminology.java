 ...
 */
//nvl changed to coalesce for postgresql on 15Apr2019
	...
	protected String doIt() throws Exception
	{
		...
		try {
			...
			sql=" 	UPDATE AD_COLUMN c"
				...
				+" 					OR coalesce(c.Description,to_nchar(' ')) <> coalesce(e.Description,to_nchar(' ')) OR coalesce(c.Help,to_nchar(' ')) <> coalesce(e.Help,to_nchar(' '))))";
			no = DB.executeUpdate(sql, false, get_TrxName());	  	
			log.info("  rows updated: "+no);
			trx.commit(true);
			...
			sql=" 	UPDATE AD_FIELD f"
				...
				+" 				  AND (f.Name <> e.Name OR coalesce(f.Description,to_nchar(' ')) <> coalesce(e.Description,to_nchar(' ')) OR coalesce(f.Help,to_nchar(' ')) <> coalesce(e.Help,to_nchar(' '))))";
			no = DB.executeUpdate(sql, false, get_TrxName());	  	
			log.info("  rows updated: "+no);
			trx.commit(true);
			...
			sql="UPDATE AD_FIELD_TRL trl"
				...
				+"		  AND (trl.Name <> e.Name OR coalesce(trl.Description,to_nchar(' ')) <> coalesce(e.Description,to_nchar(' ')) OR coalesce(trl.Help,to_nchar(' ')) <> coalesce(e.Help,to_nchar(' '))))";
			no = DB.executeUpdate(sql, false, get_TrxName());	  	
			log.info("  rows updated: "+no);
			trx.commit(true);
			...
			sql="UPDATE AD_FIELD f"
				...
				+" 		WHERE f.AD_Column_ID=c.AD_Column_ID"
				+" 		  AND c.AD_Element_ID=e.AD_Element_ID AND c.AD_Process_ID IS NULL"
				+" 		  AND (f.Name <> e.PO_Name OR coalesce(f.Description,to_nchar(' ')) <> coalesce(e.PO_Description,to_nchar(' ')) OR coalesce(f.Help,to_nchar(' ')) <> coalesce(e.PO_Help,to_nchar(' ')))"
			...
			sql=" UPDATE AD_FIELD_TRL trl"
				...
				+" 	  AND trl.AD_LANGUAGE=e.AD_LANGUAGE"
				+" 	  AND f.IsCentrallyMaintained='Y' AND f.IsActive='Y'"
				+" 	  AND (trl.Name <> e.PO_Name OR coalesce(trl.Description,to_nchar(' ')) <> coalesce(e.PO_Description,to_nchar(' ')) OR coalesce(trl.Help,to_nchar(' ')) <> coalesce(e.PO_Help,to_nchar(' ')))"
				+" 	  AND e.PO_Name IS NOT NULL)"
			...
			sql="UPDATE AD_FIELD f"
				...
				+" 		AND (f.Name<>p.Name OR coalesce(f.Description,to_nchar(' '))<>coalesce(p.Description,to_nchar(' ')) OR coalesce(f.Help,to_nchar(' '))<>coalesce(p.Help,to_nchar(' '))))";
			no = DB.executeUpdate(sql, false, get_TrxName());	  	
			log.info("  rows updated: "+no);
			trx.commit(true);
			...
			sql="UPDATE AD_FIELD_TRL trl"
				...
				+" 		AND f.AD_Field_ID=trl.AD_Field_ID AND p.AD_LANGUAGE=trl.AD_LANGUAGE"
				+" 		AND f.IsCentrallyMaintained='Y' AND f.IsActive='Y'"
				+" 		AND (trl.Name<>p.Name OR coalesce(trl.Description,to_nchar(' '))<>coalesce(p.Description,to_nchar(' ')) OR coalesce(trl.Help,to_nchar(' '))<>coalesce(p.Help,to_nchar(' '))))";
			no = DB.executeUpdate(sql, false, get_TrxName());	  	
			log.info("  rows updated: "+no);
			trx.commit(true);
			...
			sql="UPDATE AD_PROCESS_PARA_TRL trl"
				...
				+" 			  AND e.ColumnName=f.ColumnName AND f.AD_Process_Para_ID=trl.AD_Process_Para_ID"
				+" 			  AND f.IsCentrallyMaintained='Y' AND f.IsActive='Y'"
				+" 			  AND (trl.Name <> et.Name OR coalesce(trl.Description,to_nchar(' ')) <> coalesce(et.Description,to_nchar(' ')) OR coalesce(trl.Help,to_nchar(' ')) <> coalesce(et.Help,to_nchar(' '))))";
			no = DB.executeUpdate(sql, false, get_TrxName());	  	
			log.info("  rows updated: "+no);
			trx.commit(true);
			...
			sql="UPDATE AD_WF_NODE n"
				...
				+" 		WHERE w.AD_Window_ID=n.AD_Window_ID"
				+" 		  AND (w.Name <> n.Name OR coalesce(w.Description,to_nchar(' ')) <> coalesce(n.Description,to_nchar(' ')) OR coalesce(w.Help,to_nchar(' ')) <> coalesce(n.Help,to_nchar(' '))))";
			no = DB.executeUpdate(sql, false, get_TrxName());	  	
			log.info("  rows updated: "+no);
			trx.commit(true);
			...
			sql="UPDATE AD_WF_NODE_TRL trl"
				...
				+" 		  AND (trl.Name <> t.Name OR coalesce(trl.Description,to_nchar(' ')) <> coalesce(t.Description,to_nchar(' ')) OR coalesce(trl.Help,to_nchar(' ')) <> coalesce(t.Help,to_nchar(' '))))";
			no = DB.executeUpdate(sql, false, get_TrxName());	  	
			log.info("  rows updated: "+no);
			trx.commit(true);
			...
			sql="UPDATE AD_WF_NODE n"
				...
				+" 		WHERE f.AD_Form_ID=n.AD_Form_ID"
				+" 		  AND (f.Name <> n.Name OR coalesce(f.Description,to_nchar(' ')) <> coalesce(n.Description,to_nchar(' ')) OR coalesce(f.Help,to_nchar(' ')) <> coalesce(n.Help,to_nchar(' '))))";
			no = DB.executeUpdate(sql, false, get_TrxName());	  	
			log.info("  rows updated: "+no);
			trx.commit(true);
