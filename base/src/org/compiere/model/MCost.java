...
public class MCost extends X_M_Cost
{
		...
		boolean zeroCostsOK, String trxName)
	{
		...
		BigDecimal OtherCostAmt = Env.ZERO; // added on 22-Feb-11
		...
		String sql = "SELECT"
			...
			+ " sum(c.currentqty) " // added on 22-Feb-11
			+ " FROM M_Cost c"
		...
		try
		{
			...
			while (rs.next ())
			{
				...
				BigDecimal currentqty = rs.getBigDecimal(7); // added on 22-Feb-11
...
//						otherCostEach = otherCostEach.add(currentCostPrice).add(currentCostPriceLL);
					if (M_ASI_ID > 0)
						OtherCostAmt = OtherCostAmt.add((currentCostPrice.multiply(currentqty.min((qty).abs()))).multiply(multiplier)); // added on 22-Feb-11
					else
						OtherCostAmt = OtherCostAmt.add(currentCostPrice.multiply(qty)); // added on 22-Feb-11
		...
		//	Other Costs
//		BigDecimal otherCost = otherCostEach.multiply(qty); //Commented on 22-Feb-11


		//	Costs
//		BigDecimal costs = otherCost.add(materialCost);//Commented on 22-Feb-11
		BigDecimal costs = OtherCostAmt.add(materialCost);//added on 22-Feb-11
		if (costs.signum() == 0)
			return null;
