...
public class MInvoiceLine extends X_C_InvoiceLine
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		{
			//	Prevent duplicated items in lines
			boolean isDuplicated = false;
			MInvoiceLine[] invoiceLines = getParent().getLines();
			for (MInvoiceLine oLine : invoiceLines)
			{
			...
			if (isDuplicated)
			{
				log.saveError("Error", Msg.getMsg(getCtx(), "Duplicated Products in the invoice: " + getParent().getDocumentNo()));
				return false;
			}
		}
		*/
