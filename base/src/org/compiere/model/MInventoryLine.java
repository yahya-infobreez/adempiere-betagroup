...
public class MInventoryLine extends X_M_InventoryLine 
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		//Code to validate Trx.qty Vs On Hand Qty ********** 20jun2010
				MProduct product = MProduct.get(getCtx(), getM_Product_ID());
				...
				if (getQtyInternalUse().compareTo(QtyOnHand) == 1)
				{
					log.saveError("Error", "Trx.Qty (" + getQtyInternalUse() + ") should not be greater than On Hand Qty (" + QtyOnHand + ")");
					return false;
					
				}
	
				
         //Validate Trx.Qty>0   ***********19Jul2011
				BigDecimal TrQty = (BigDecimal)getQtyInternalUse();
				
				if (TrQty.doubleValue() ==0 )
				{
					log.saveError("Error", Msg.getMsg(getCtx(), "Qty Should not be Zero" ));
					return false;
				}	
		...
		//Validate Product with Prod.Category Vs Warehouse  Dated:21Jul2011
				...
				Integer ParentId = (Integer)getM_Inventory_ID();
				BigDecimal Sqlcnt = Env.ZERO;
				BigDecimal Wid = Env.ZERO;
				
				String SqlWH = "Select M_Warehouse_ID from M_Inventory where M_Inventory_ID = ?";
				try {
