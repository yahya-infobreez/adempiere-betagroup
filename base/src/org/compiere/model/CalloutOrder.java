...
public class CalloutOrder extends CalloutEngine
{
	...
	public String bPartner (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		String sql = "SELECT p.AD_Language,p.C_PaymentTerm_ID,"
			...
			+ " p.InvoiceRule,p.DeliveryRule,p.FreightCostRule,p.DeliveryViaRule,"
	...
	public String product (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		{
			String WinHdr=Env.getHeader(ctx, WindowNo);
	...
	public String amt (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		boolean IsSO = "Y".equals(Env.getContext(Env.getCtx(), WindowNo, "IsSOTrx"));
		if (IsSO) 
			return "";
	...
	public String BPCheck(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		if ((Integer)mTab.getValue("C_BPartner_ID")!= null)
		{
			...
			if (IsSOTrx)
				...
				//sql= "SELECT Value from C_BPartner where (IsVendor='Y' or IsEmployee='Y') and C_BPartner_ID= ? " ;
				sql= "SELECT Value from C_BPartner where IsVendor='Y' and C_BPartner_ID= ? " ;
			PreparedStatement pstmt = null;
			ResultSet rs = null;
			try
			{
	...
	public String SetDiscount(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		if ((Integer)mTab.getValue("M_Product_ID")!= null & IsSOTrx)
		{
			...
			{	
				...
				{
					Docid = (BigDecimal)rs.getBigDecimal("C_DOCTYPETARGET_ID");
					Disc = (BigDecimal)rs.getBigDecimal("DISCPERC");
				}
			}
			catch(Exception e){
			}
			...
			sql = "SELECT * FROM C_DOCTYPE WHERE DOCSUBTYPESO='OB' AND C_DOCTYPE_ID=?" ;
			PreparedStatement pstmt1 = null;
			ResultSet rs1 = null;
			try
			{
				pstmt1 = DB.prepareStatement(sql, null);
				pstmt1.setBigDecimal(1, Docid);
	...
	public String SetAmt(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		if (IsSO) 
		{
			...
			BigDecimal DocID = DB.getSQLValueBD(null, SqlDocType, OrdID);
			String SqlSOType = "Select nvl(DOCSUBTYPESO,'SO') from c_doctype where c_doctype_id = ?";
			String SOType = DB.getSQLValueString(null, SqlSOType, DocID);
			
			if (SOType.equalsIgnoreCase("OB"))
			{
				BigDecimal Qty,Discount,Price,NetAmt,DiscAmt,ActPrice;
				Qty = (BigDecimal)mTab.getValue("QtyEntered");
				Discount = (BigDecimal)mTab.getValue("Discount");
				Price = (BigDecimal)mTab.getValue("PriceList");
				
				//Price = Price.setScale(0, BigDecimal.ROUND_CEILING);
				//mTab.setValue("PriceList",Price);
				
				DiscAmt = new BigDecimal ((Price.doubleValue()*Discount.doubleValue()) / 100.00) ;
				ActPrice =  new BigDecimal (Price.doubleValue() - DiscAmt.doubleValue());
				ActPrice = ActPrice.setScale(2,BigDecimal.ROUND_HALF_UP);
				mTab.setValue("PriceActual", ActPrice);
				mTab.setValue("PriceEntered", ActPrice);
				
				NetAmt = ActPrice.multiply(Qty);
				NetAmt = NetAmt.setScale(2, BigDecimal.ROUND_HALF_UP);
				mTab.setValue("LineNetAmt", NetAmt);     
			}
			else
			{
				BigDecimal Qty,NetAmt,ActPrice;
				Qty = (BigDecimal)mTab.getValue("QtyEntered");
				ActPrice = (BigDecimal)mTab.getValue("PriceActual");
				NetAmt = ActPrice.multiply(Qty);
				NetAmt = NetAmt.setScale(2, BigDecimal.ROUND_HALF_UP);
				mTab.setValue("LineNetAmt", NetAmt);
			}
	...
	public String SetIsBom(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		if (IsSO) 
		{
			...
			if (SOType.equalsIgnoreCase("OB"))
			{
				//Check IsBom in Product window and set the field in order line field
				int M_Product_ID = Env.getContextAsInt(ctx, WindowNo, "M_Product_ID");
				
				try 
				{
					String sqlIsBom = "Select nvl(ISBOM,'N') from m_product where m_product_id = ?";
					String strIsBom = DB.getSQLValueString(null, sqlIsBom, M_Product_ID);
					if (strIsBom.equalsIgnoreCase("Y"))
					{
						mTab.setValue("IsBOM","Y");
						...
						mTab.setValue("IsBOM","N");
				} catch (Exception e) { }
				//Set GetPrice to 'N'
				mTab.setValue("GETPRICE", "N");
