...
public class MInOutLine extends X_M_InOutLine
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		//********BETA CODE 20jun2010   SHIPMENT QTY VS ORDER QTY*********
		
//		if (getC_OrderLine_ID() != 0 && getM_RMALine_ID() == 0)
//		{
//			MOrderLine o1 = new MOrderLine(getCtx(), getC_OrderLine_ID(), get_TrxName());
//			BigDecimal MaxMovementQtyAllowed = o1.getQtyOrdered().subtract(o1.getQtyDelivered());
//			if (getMovementQty().compareTo(MaxMovementQtyAllowed) ==1)
//			{
//				if (getParent().isSOTrx())
//				{
//					log.saveError("Error","Shipment Qty should not exceed Ordered Qty");
//					return false;
//				}
//			}
//		}
		//*******BETA CODE END   SHIPMENT QTY VS ORDER QTY****************
		
		
		
		
		
		//********BETA CODE 20jun2010   SHIPMENT QTY VS QTY ON HAND*********
//		if (getC_OrderLine_ID() != 0 && getM_RMALine_ID() == 0)
//		{
//			MProduct product = MProduct.get(getCtx(), getM_Product_ID());
//			MLocator locator1 = MLocator.get(getCtx(), getM_Locator_ID());
//			String sql = "SELECT SUM(ps.QtyOnHand) FROM M_Product_Stock_v ps" +
//					" WHERE ps.value = '" + product.getValue() + "'" +
//					" AND ps.M_Warehouse_ID = " + locator1.getM_Warehouse_ID();
//		
...
//			{
//				if (getParent().isSOTrx())
//				{				log.saveError("Error","Shipment Qty should not exceed Qty on Hand" + "\n" +
...
//		}
		...
		//*******BETA CODE END   SHIPMENT QTY VS QTY ON HAND****************
		...
		if (newRecord || is_ValueChanged("M_Product_ID"))
		{
			...
			MInOutLine[] inoutLines = getParent().getLines();
			for (MInOutLine oLine : inoutLines)
			{
			...
			if (isDuplicated)
			{
				log.saveError("Error", Msg.getMsg(getCtx(), "Duplicated Products found in the transaction: " + getParent().getDocumentNo()));
				return false;
			}
		}
