...
public class MCostDetail extends X_M_CostDetail
{
		...
		else if (getC_InvoiceLine_ID() != 0)
		{
			...
			else if (!ce.isCostingMethod())		//	Cost Adjustments
			{
					...
					MAcctSchema.COSTINGMETHOD_AverageInvoice.equals(costingMethod))
				{
					...
					/** added on 23Feb11 to set zeroqty for landed cost and add this landedcost to mat.cost*/
						
						String sql1 = " select currentqty,currentqty*currentcostprice as prodamount " +
								" from m_cost" +
								" where m_costelement_id in (select m_costelement_id" +
								" from m_costelement where costingmethod = 'A')" +
								" and m_product_id = ? and ad_client_id = " + cost.getAD_Client_ID();
						
						PreparedStatement pstmt = null;
						ResultSet rs = null;
						BigDecimal prod_amt =Env.ZERO;
						BigDecimal prod_qty =Env.ZERO;
						try
						{
							...
							if (rs.next ())
							{
								prod_qty = rs.getBigDecimal(1);
								prod_amt = rs.getBigDecimal(2);
								BigDecimal NewAmt = prod_amt.add(amt);
								NewAmt = NewAmt.setScale(4,BigDecimal.ROUND_HALF_UP);
								BigDecimal NewRate = NewAmt.divide(prod_qty,4,BigDecimal.ROUND_HALF_UP);
								NewRate = NewRate.setScale(4,BigDecimal.ROUND_HALF_UP);
								String sql2 = " update m_cost set currentcostprice = " + NewRate + 
								 " where m_costelement_id in (select m_costelement_id" +
								" from m_costelement where costingmethod = 'A')" +
								 " and m_product_id = " + cost.getM_Product_ID()+ " and ad_client_id = " + cost.getAD_Client_ID();
								
								int upd1 = DB.executeUpdate(sql2,null);
								
								cost.setCurrentQty(Env.ZERO);
								cost.setCurrentCostPrice(Env.ZERO);
								/*String sql3 = " update m_cost set currentcostprice = " + NewRate + 
								 " where m_costelement_id in (select m_costelement_id" +
								" from m_costelement where costingmethod is null)" +
								 " and m_product_id = " + cost.getM_Product_ID()+ " and ad_client_id = " + cost.getAD_Client_ID();
								
								int upd2 = DB.executeUpdate(sql3,null);*/
							}
						}
	...
	/*{origanl commented out on 23feb2011
					...
					for (int i = 0 ; i < lce.length ; i++)
					{
						...
						if  (lCost != null)
						{
							if (qtyOnhand.signum() != 0)
							{
								...
								// new average cost
								BigDecimal oldSum = lCost.getCurrentCostPrice().multiply(lCost.getCurrentQty().min(qty));
							...
							lCost.setCurrentQty((lCost.getCurrentQty().add(qty)).max(Env.ZERO));
							if (!lCost.save())
							{
